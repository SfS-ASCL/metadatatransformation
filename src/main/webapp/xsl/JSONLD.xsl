  ...
  <xsl:template name="JSONLD">
	...
	<xsl:text>/*&lt;![CDATA[*/</xsl:text>
  	...
  	<xsl:if
  		...
  		<xsl:if
  			...
  			<xsl:for-each select="$CreatorPers/*[local-name() = 'Person']">
  				...
  				<xsl:text>            "givenName": "</xsl:text><xsl:value-of select="./*[local-name() = 'firstName']"/><xsl:text>",&#xA;</xsl:text>
  				<xsl:text>            "familyName": "</xsl:text><xsl:value-of select="./*[local-name() = 'lastName']"/><xsl:text>",&#xA;</xsl:text>
  				<xsl:text>            "name": "</xsl:text><xsl:value-of select="concat(./*[local-name()='firstName'], ' ', ./*[local-name()='lastName'])"/><xsl:text>"&#xA;</xsl:text>
  				<xsl:text>        }</xsl:text>
  			</xsl:for-each>
  		</xsl:if>
  		<xsl:text>&#xA;    ],&#xA;</xsl:text>
  	...
  	<xsl:if test="//*[local-name() = 'OriginalSource'] != ''">
  		...
  		<xsl:text>        "name": "</xsl:text><xsl:value-of select="//*[local-name() = 'OriginalSource']"/><xsl:text>"&#xA;</xsl:text>
  		<xsl:text>    },&#xA;</xsl:text>
  	</xsl:if>
  	...
  	<xsl:for-each select="//*[local-name()='ResourceProxy']">
  		...
  		<xsl:variable name="ResProxListInfo" select="//*[text()=$ResFileName]/parent::node()"/>
  		
  		<xsl:text>        {&#xA;</xsl:text>
  		<xsl:text>           "@type": "DataDownload",&#xA;</xsl:text>
  		<xsl:text>           "contentURL": "</xsl:text><xsl:value-of select="$ResName"/><xsl:text>",&#xA;</xsl:text>
  		<xsl:text>           "encodingFormat": "</xsl:text><xsl:value-of select="./*[local-name()='ResourceType']/@mimetype"/><xsl:text>"&#xA;</xsl:text>
  		
  		<xsl:if test="$ResProxListInfo//*[local-name()='Size'] != ''">
  			<xsl:text>,&#xA;</xsl:text>
  			<xsl:text>           "contentSize": "</xsl:text><xsl:value-of select="$ResProxListInfo//*[local-name()='Size']"/><xsl:text>",&#xA;</xsl:text>
  			...
  			<xsl:text>                   "identifier": "md5:</xsl:text><xsl:value-of select="$ResProxListInfo//*[local-name()='md5']"/><xsl:text>",&#xA;</xsl:text>
  			<xsl:text>                   "value": "</xsl:text><xsl:value-of select="$ResProxListInfo//*[local-name()='md5']"/><xsl:text>"&#xA;</xsl:text>
  			<xsl:text>               },&#xA;</xsl:text>
  			...
  			<xsl:text>                   "identifier": "sha1:</xsl:text><xsl:value-of select="$ResProxListInfo//*[local-name()='sha1']"/><xsl:text>",&#xA;</xsl:text>
  			<xsl:text>                   "value": "</xsl:text><xsl:value-of select="$ResProxListInfo//*[local-name()='sha1']"/><xsl:text>"&#xA;</xsl:text>
  			<xsl:text>               },&#xA;</xsl:text>
  			...
  			<xsl:text>                  "identifier": "sha1:</xsl:text><xsl:value-of select="$ResProxListInfo//*[local-name()='sha256']"/><xsl:text>",&#xA;</xsl:text>
  			<xsl:text>                  "value": "</xsl:text><xsl:value-of select="$ResProxListInfo//*[local-name()='sha256']"/><xsl:text>"&#xA;</xsl:text>
  			<xsl:text>               }&#xA;</xsl:text>
